...
def process_args(args) -> None:
    ...
    """
    debug('# Running MCS Alert script')

    # Read args
    alert_file_location: str = args[ALERT_INDEX]
    webhook: str = args[WEBHOOK_INDEX]
    token: str = args[WEBHOOK_KEY]
...
def generate_msg(alert: any, options: any) -> any:
    ...
    if 'agent' in alert:
        host = '({0}) - {1}'.format(alert['agent']['id'], alert['agent']['name'])
    if 'agentless' in alert:
        host = alert['agentless']['host'],
    ...
    msg = {
      ...
      "event": f"WAZUH {severity} Alert",
      "group": "Security",
      "origin": alert['location'],
      "resource": host,
      "service": f"Alert level: {level}",
      "severity": f"{severity}",
      "text": alert.get('full_log', ''),
      "type": "exceptionAlert",
      "value": alert['rule']['description'] if 'description' in alert['rule'] else 'N/A',
      "status": "open",
      "url": "https://security.vels.online/",
    }

    return json.dumps(msg)


def send_msg(msg: str, url: str, token: str) -> None:
